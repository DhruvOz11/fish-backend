...
} catch (e) { console.warn('Rate limiter not loaded:', e.message); }
...
function safeRoute(path, file) {
  try {
    ...
    app.use(path, (req, res) => res.status(503).json({ success: false, message: `Route ${path} unavailable: ${e.message}` }));
  }
}
...
app.get('/api/health', (req, res) => {
  res.json({ success: true, message: 'FreshCatch API running', db: mongoose.connection.readyState === 1 ? 'connected' : 'disconnected', timestamp: new Date().toISOString() });
});
...
mongoose.connect(process.env.MONGODB_URI)
  .then(() => {
    ...
    app.listen(PORT, () => console.log(`🚀 FreshCatch: http://localhost:${PORT}`));
  })
